#!/usr/bin/env bun
/**
 * Gigi CLI Entry Point
 *
 * Usage:
 *   gigi start [--port N] [-d|--detach]   Start the daemon
 *   gigi stop [--port N]                  Stop the running daemon
 *   gigi status                           Show daemon status
 *   gigi uninstall                        Remove GIGI (detects install method)
 *   gigi doctor                           Check environment & connectivity
 *   gigi version                          Print version
 *   gigi help                             Show this help
 *
 * First-time setup happens in the dashboard at http://localhost:3142
 * after `gigi start` — there is no longer a CLI wizard.
 */
...
function printHelp(): void {
  console.log(`
${c.cyan('Gigi')} ${c.dim(`v${getVersion()}`)}
Just A Rather Very Intelligent System

${c.bold('Usage:')}
  gigi <command> [options]

${c.bold('Commands:')}
  ${c.cyan('start')}     Start the GIGI daemon
  ...
  ${c.cyan('update')}    Update GIGI (dispatches based on install method)
  ${c.cyan('uninstall')} Remove GIGI (dispatches based on install method)
...
${c.bold('Start options:')}
  ...
  --data-dir <path> Override data directory (default: ~/.gigi)
...
${c.bold('Examples:')}
  gigi start                  Start in foreground
  gigi start -d               Start as background daemon
  gigi start --port 8080      Start on custom port
  gigi restart                Restart with same settings
  gigi logs -f                Follow live log output
  gigi update                 Update to latest version
  gigi uninstall              Remove GIGI from this machine
  gigi doctor                 Check if everything is working
`);
}

function assertSupportedPlatform(): void {
  if (process.platform !== 'win32') return;
  console.error(c.red('Native Windows installs are not supported for the GIGI daemon.'));
  console.error(c.dim('Use WSL2 for the Bun install, or run GIGI with Docker on Windows.'));
...
async function cmdStart(args: string[]): Promise<void> {
  ...
  const cfgPath = join(homedir(), '.gigi', 'config.yaml');
  if (!_exists(cfgPath)) {
  ...
  if (!detach) {
    ...
    if (!acquireLock(process.pid)) {
      console.log(c.yellow('GIGI is already running'));
      console.log(c.dim('  Stop it first with: gigi stop'));
      process.exit(1);
    }
    ...
    if (existingPid) {
      console.log(c.yellow(`GIGI is already running (PID ${existingPid})`));
      console.log(c.dim('  Stop it first with: gigi stop'));
      process.exit(1);
    }

    // Run in background — spawn a detached child process with log file
    console.log(c.cyan('Starting Gigi daemon...'));

    const logPath = getLogPath();
    const logFile = Bun.file(logPath);

    const daemonArgs = [join(PACKAGE_ROOT, 'bin/gigi.ts'), 'start', '--no-open'];
    ...
    if (runningPid) {
      console.log(c.green(`✓ GIGI daemon started (PID ${runningPid})`));
      console.log(c.dim(`  Dashboard: http://localhost:${port ?? 3142}`));
      console.log(c.dim(`  Logs:      ${logPath}`));
      console.log(c.dim(`  Stop with: gigi stop`));
...
async function cmdStop(args: string[] = []): Promise<void> {
  ...
  if (!pid) {
    ...
    if (cleanup.terminated.length > 0 || cleanup.forced.length > 0) {
      ...
      console.log(c.green(`✓ GIGI was not locked, but the port is now clear.${details}`));
    } else {
      console.log(c.yellow('GIGI is not running.'));
    }
    return;
  }

  console.log(c.cyan(`Stopping GIGI daemon (PID ${pid})...`));
  try {
    ...
    releaseLock();
    if (!cleanup.released) {
      console.error(c.red(`✗ GIGI stopped but port ${port} is still occupied.`));
      process.exit(1);
    }
    ...
    const details = cleanup.forced.length > 0
      ...
      : cleanup.terminated.length > 0
        ? ` Cleaned up lingering listener(s) on port ${port}: ${cleanup.terminated.join(', ')}.`
        : '';
    console.log(c.green(`✓ GIGI daemon stopped.${details}`));
  } catch (err) {
...
function cmdStatus(): void {
  const pid = isLocked();
  if (pid) {
    console.log(`${c.green('●')} GIGI is ${c.green('running')} (PID ${pid})`);
    ...
    console.log(`${c.red('●')} GIGI is ${c.red('stopped')}`);
    console.log(c.dim(`  Start with: gigi start`));
  }
}
...
function cmdLogs(args: string[]): void {
  ...
  if (!existsSync(logPath)) {
    console.log(c.yellow('No log file found. Start the daemon first: gigi start'));
    return;
  }
...
switch (command) {
  ...
  case 'onboard':
    ...
    console.log(c.dim('    1. Run: gigi start'));
  ...
  default:
    ...
    console.log(c.dim('Run "gigi help" for usage information.'));
    process.exit(1);
}
