...
import { createGigiRoutes } from './gigi-routes.ts';
...
export function createApiRoutes(ctx: ApiContext): Record<string, unknown> {
  return {
    ...
    // --- MCP Server endpoint ---
    // Implements the Model Context Protocol JSON-RPC 2.0 transport so that
    // external MCP clients (e.g. OpenHuman) can discover and call Jarvis tools.
    //
    // POST /api/mcp  — accepts a JSON-RPC request body, returns a JSON-RPC response
    // GET  /api/mcp  — returns an SSE-compatible event-stream stub required by the
    //                  MCP spec to signal that the server supports streaming
    '/api/mcp': {
      GET: () => {
        // Minimal SSE handshake — clients that probe GET to check for streaming
        // support will receive HTTP 200 with the correct content-type.
        return new Response(': MCP SSE endpoint (Jarvis)\n\n', {
          status: 200,
          headers: {
            ...CORS,
      ...
      POST: async (req: Request) => {
        // Resolve the live ToolRegistry from the agent service
        const registry = ctx.agentService
          .getOrchestrator()
          .getToolRegistry();
          ...
          return json(
            {
              jsonrpc: '2.0',
              id: null,
              error: { code: -32603, message: 'Tool registry not yet initialised' },
            },
            503,
          );
        }
        ...
        const response = await handleMcpRequest(body, registry);
        const hasError = 'error' in response;
        return json(response, hasError ? 400 : 200);
      },
    },
