...
describe('CLI --version output', () => {
  ...
  // Gigi renamed from jarvis — point at the gigi CLI binary
  const GIGI_BIN = existsSync(join(REPO_ROOT, 'bin', 'gigi.ts'))
    ? join(REPO_ROOT, 'bin', 'gigi.ts')
    : join(REPO_ROOT, 'bin', 'jarvis.ts');

  async function runCli(args: string[]): Promise<{ exitCode: number; stdout: string }> {
    const proc = Bun.spawn(['bun', GIGI_BIN, ...args], {
      stdout: 'pipe',
      stderr: 'pipe',
      cwd: REPO_ROOT,
    });
