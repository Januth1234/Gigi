...
function isPublicRoute(pathname: string, method: string): boolean {
  return (
    ...
    // MCP OPTIONS pre-flight must be reachable without a session cookie
    (pathname === '/api/mcp' && method === 'OPTIONS') ||
    method === 'OPTIONS'
  );
}
    ...
    this.server = Bun.serve<{ sidecar_id?: string; proxy_target?: string; _proxyUpstream?: WebSocket }>({
      ...
      async fetch(req, server) {
        ...
        if (self.authToken && !isPublicRoute(pathname, req.method)) {
          ...
          // Also accept Authorization: Bearer <token> — used by MCP clients
          // (OpenHuman, curl, other programmatic callers) that cannot set cookies.
          const authHeader  = req.headers.get('Authorization') ?? '';
          ...
          // ?token= in the URL is the browser one-time bootstrap flow:
          // exchange it for a session cookie and redirect to the clean URL.
          // This is handled first (before other auth checks) so that browser
          // requests always go through the cookie exchange regardless of
          // whether a bearer token is also present.
          ...
          const authed =
            (cookieToken && safeCompare(cookieToken, self.authToken)) ||
            (bearerToken && safeCompare(bearerToken, self.authToken));
          ...
          if (req.method === 'OPTIONS') {
            ...
            return new Response(null, {
              ...
              headers: {
                ...
                // Include MCP protocol headers so MCP clients can negotiate
